...
public class DeviceType {  
	...
	@Post
	public JsonA info(){
		String tpl = "select * from base_device_type order by deptid";
	...
	public String delsysfunc(String node, String parents, CookieSession cs){
		try{
			...
			DB3.update(Template.apply("delete from base_device_type where deptid=${node}",node));
			String tpl = "select * from base_device_type where parents=${parents}";
			JsonA jsons = DB3.getResultAsJsonA(Template.apply(tpl,parents), "");
			if(jsons.size() == 0){
				DB3.update(Template.apply("update base_device_type set leaf='true' where deptid=${parents}",parents));
			}
			return "success";
		}catch(Exception e){
			return "fail";			
	...
	public String savefunc(String jstr, CookieSession cs){
		...
		try{
			...
			DB3.update(Template.apply("update base_device_type set leaf='false' where deptid=${parents}",parents));
			return "success";
		}catch(Exception e){
			return "fail";			
		}
	...
	public String sysfuncNID(){
		return DB3.getNewID("base_device_type", 1);
	}
